...
begin
  ...
  agg as (
    select
      ...
      count(distinct visitor_id)::bigint                 as t_unique_visitors,
      coalesce(sum(pageviews),0)::bigint                 as t_pageviews,
      coalesce(sum(engagement_ms),0)::bigint             as t_eng_ms_total,
      count(*) filter (where engagement_ms > 0)::bigint  as t_eng_ms_n,
      count(*) filter (where is_bounce)::bigint          as t_bounced,
      coalesce(sum(conversions),0)::bigint               as t_conversions,
      count(*) filter (where converted)::bigint          as t_converted_sessions
    from filt
  )
  select
